...
date: '2025-11-24'
description: "Testing context-based optimization for agent continual learning. Pareto-optimal cost-performance, a humbling lesson in overfitting, and what I'd try next."
---

![grok exo](/images/exo/exo-grok-2.png)

**Testing context-based optimization for agent continual learning. Pareto-optimal cost-performance, a humbling lesson in overfitting, and what I'd try next.**

---

### TL;DR

Continual learning — agents that improve from experience — keeps surfacing as a key capability gap. Weight updates are prohibitive at scale; context-based learning is the lightweight alternative. I built Exo to test whether context-based optimization could close the gap between economy and frontier models on [GAIA](https://huggingface.co/gaia-benchmark).
The architecture: modular ReAct (reflection → domain routing → specialized action) with an adapted [GEPA](https://arxiv.org/abs/2507.19457) optimizer for trajectory-level self-improvement. Along the way, [Grok 4 Fast](https://x.ai/news/grok-4-fast/) + my tooling achieved Pareto-optimal cost-performance — higher accuracy than [o4-mini + HAL](https://hal.cs.princeton.edu/gaia) at 4x lower cost.
...
Code [here](https://github.com/heyalexchoi/exo-public)
...
- **Multi-threaded concurrency in containerized deployment:** Tasks are run in parallel, each with its own thread, agent, browser, and code executor. I chose multi-thread concurrency for maximum compatibility and reuse with [DSPy's](https://dspy.ai/) Evaluator and Optimizer. The application is Docker containerized for easy, portable, and reliable deployment.
...
**Infrastructure.** Docker container running [LangGraph](https://www.langchain.com/langgraph) to orchestrate DSPy modules, with multi-threaded eval and MLflow tracking. Tested and reusable for future agent work.
